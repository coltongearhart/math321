...
% 1) Expected value with deductible and cap
% 2) Cdf and pdf of transformation (increasing function)
% 3) Cdf and pdf of transformation (decreasing function)
% 4) Cdf and pdf of a mixed distribution (exponential)
...
\begin{enumerate}
    ...
    \item[] Since the device will not be monitored during its first 10 years of service, the time to discovery of its failure is $X = \text{max}(T, 10)$ (i.e. whichever $X$ takes the value of whichever is greater for that particular $x$ point).% Acted 9-30 (different distribution)
    \begin{enumerate}
